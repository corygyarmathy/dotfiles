...
**`automerge-nudge` is what stops stale auto-merge PRs from deadlocking on strict.** Merge queues are not available here - this repo is owned by a user account, and GitHub only offers the merge queue on organization-owned repositories (`422 invalid rule 'merge_queue'`; the UI shows no "Require merge queue" rule for the same reason). So stale auto-merge PRs are brought up to date the explicit way: a small workflow, triggered on every push to `master` and on a 15-minute cron, rebases the stale auto-merge PRs on the bot-owned `deps/*` branches - the ones `flake-update.yml` and `package-update.yml` open - onto `master` (`gh pr update-branch --rebase`), in least-recently-updated order, stopping at the first success - the merge-queue replacement that works on a user-owned repo. It rebases rather than merges because the repo prefers a clean linear PR branch, and the rebase only touches `deps/*` branches: the branches `FLAKE_UPDATE_TOKEN` actually owns, so the force-push is legitimate. Dependabot PRs (`dependabot/*` heads) are left to Dependabot, which rebases its own PRs; fork PRs and human-armed PRs on other branches are left for a human, since a force-push to a branch this token does not own is not this job's call. Two things make it safe: it uses `FLAKE_UPDATE_TOKEN`, not `GITHUB_TOKEN`, because updating a branch mints a new head commit and GitHub suppresses workflow events for anything `GITHUB_TOKEN` does - under `GITHUB_TOKEN` the required `nixos ci` check would never run on the new head and the PR would hang in "pending" instead of draining; and it stops at the first success, and one update per run, so a spreading failure cannot touch multiple branches at once and one PR that cannot be rebased does not starve the rest. It does not relax strict - it automates the step strict requires. A `deps/*` PR that conflicts on rebase is left for the next version bump to reconcile; if every candidate fails the run is red rather than silently green, so a persistently-stuck PR stays visible.

**The checks are one job per check, and the matrix naming them is audited.** `checks` is a matrix over the attribute names in `checks/default.nix`, each shard running `nix build .#checks.x86_64-linux.<name>`. It replaced a single `flake-check` job running `nix flake check --max-jobs 5`, which was as slow as the sum of its tests and had twice grown past the host build it is meant to fit inside. Sharded, the gate is bounded by the _slowest_ check (`upgrade-verify`, ~2.5m) rather than by their total, at the cost of runner-minutes that are free on a public repository.
