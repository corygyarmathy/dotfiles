...
**2. The claim marker moves from an assignee to the `ready-for-agent` label**, amending ADR 0004 §3. GitHub will not let an App hold an issue assignment, so §3's `gh issue edit <n> --add-assignee @me` is unavailable to this identity. The replacement is already in place: the runner's frontier query is scoped by `ready-for-agent`, so removing that label _is_ the claim. A ticket the runner hands back comes back as `agent-stuck`, not as this label (plan item 8): re-adding the claim marker would send a ticket the runner could not finish straight round the frontier query again, to burn its retry budget on the same failure every poll. This leaves the assignee convention in `docs/agents/issue-tracker.md` untouched for humans reading the same tracker, which matters more than it looks - a shared tracker with two claim mechanisms would be worse than either.
