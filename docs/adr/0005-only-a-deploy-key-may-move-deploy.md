...
**Narrowed 2026-09-09 by [ADR 0006](0006-the-runner-has-its-own-github-account.md), which reverses ADR 0004 §4.** The sentence above buys its reach from the single account: one API field could speak for every credential here only because every credential here was the same user. Once the runner has its own account that is no longer an argument about `AFK_AGENT_TOKEN`, and `current_user_can_bypass` answered for `corygyarmathy` says nothing about it. The decision is unaffected and the conclusion is if anything firmer - this ruleset exempts a deploy key and nothing else, and a `write` collaborator is not one - but it is now two facts rather than one, so the reissued token is checked against `deploy` in its own right (`GET /rules/branches/deploy`, under that token, must still list `update`).
