...
Only three packages qualify. `comskip` tracks an upstream tag and `obsidian-headless` an npm release; `quartz` has ~42 npm-pinned plugins that nothing else would ever move. _(2026-08-24: the digital garden moved to Hugo, which is in nixpkgs and rides `flake.lock` like anything else. `packages/quartz` and its updater are gone, so two packages qualify now — the reasoning below stands, it simply has one fewer subject.)_ The remaining packages are our own code, where the version string tracks nothing. `argtable2` is deliberately left out: upstream last moved in 2024 and its tag namespace contains `vjonathanmarvens-0.1.1`, so an automated updater has more ways to go wrong than right.

Quartz's own release is _not_ automated, only its plugins. Its derivation carries a page of workarounds tied to how v5 resolves plugins, and getting that wrong produces a build that succeeds and a site that is silently featureless - precisely the failure the CI gate cannot see. _(This is the paragraph that eventually decided against Quartz: a package that cannot be updated safely is one that stops being updated, which is not a position to hold a public-facing site in indefinitely.)_
