...
Each decision below is stated without its implementation parameters - counts, prefixes, path lists, option names. Those live in `modules/services/afk-agent.nix`, which is expected to change; this document is not. See `docs/agents/domain.md` for the convention and why it exists.
...
**4. PRs are opened under a second fine-grained PAT scoped to this repo (`AFK_AGENT_TOKEN`)**, not `GITHUB_TOKEN`, following the exact shape of `FLAKE_UPDATE_TOKEN`: GitHub suppresses workflow events raised by `GITHUB_TOKEN`, so the required `nixos ci` check would never fire on the PR. Not a separate GitHub account either - a dedicated branch prefix and label carry the same at-a-glance distinction `deps/*` already provides for `FLAKE_UPDATE_TOKEN`, without a second account's credentials and 2FA to manage. The prefix and label names are parameters, owned by `modules/services/afk-agent.nix`.
...
**6. Implementation retries in the same session, review does not.** A retry against a failing build or test happens inside the session that produced the failure, because a retry without the prior failure as context is close to useless. Review is always a separate pass in a fresh context (the existing `/code-review` skill, standards + spec in parallel) after implementation succeeds, regardless of how many retries it took - a self-review in the context that just wrote the code is the weakest form. A ticket that exhausts its retries, or otherwise can't proceed, gets a comment, a relabel, and a notification - never a PR. The retry budget is a parameter, owned by `modules/services/afk-agent.nix`, bounded so that it cannot meaningfully threaten OpenCode Go's usage caps.

**7. The whole thing is a real NixOS module**, not a script someone remembers to run, so turning it off in an emergency is one boolean - following the pattern this repo already uses for every other service. The module path and option name follow that existing convention rather than being decided here - see `modules/services/afk-agent.nix`.
