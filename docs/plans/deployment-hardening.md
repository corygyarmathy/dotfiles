...
Two things came with it, both to replace something `nix flake check` was doing incidentally. The matrix names its shards rather than discovering them, because a discovery job serialises ahead of every shard and eats most of the saving - so the `lint` job asserts that the matrix and `checks/default.nix` list the same names, since a check dropped from the matrix would otherwise stop running silently. And `lint` evaluates `devShells`, `packages` and `apps` by attribute, because building checks by attribute path no longer evaluates them and noticing when one stops evaluating was worth keeping.

The obvious way to write that second one - `nix flake check --no-build` - is wrong here, and passed locally before failing in CI. Stylix reads a base16 scheme through import-from-derivation, so evaluating any host needs that derivation built, and `--no-build` forbids it. Locally the path is already in the store from the last host build, so the command succeeds; on a clean runner it fails with `path '...-base16-schemes-....drv' is not valid`. Worth recording as a shape rather than a fact about stylix: **a verification whose result depends on what is already in the local store is not a verification**, and this one looked green twice before a clean runner disagreed.
