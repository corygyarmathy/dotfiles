...
### Revised 2026-09-10: a dying run's work is kept, not deleted

The teardown deleted the branch of a run that never pushed, on the reasoning above that nothing of a dead run is worth having. The first live week disproved it. A run implemented its ticket, wrote six new harness cases for it, got all six passing, and was killed by the kernel's OOM killer while proving it (see item 4's `maxMemory` note for the memory half); the next poll's guard found the worktree, saw no pull request and an unpushed branch, and deleted both - 1462 insertions of tested work, gone, because the model had not reached its commit yet.

**Tear-down rather than resume is unchanged**, and is still right for the reason it was written: the dead run's prompt, logs and attempt count did not survive it, and resuming unattended work nobody can vouch for is what ADR 0004 §6 exists to prevent. What was wrong was treating that as a reason to *destroy* the work as well. Keeping is not resuming. So before either hand-back tears anything down, whatever is in the worktree is committed onto its branch as a single `WIP` commit whose first line says it has passed no gate, and the branch is then kept rather than deleted. The ticket still goes to a human; the comment now tells them the branch is there, unpushed, on the runner's host.

**It is committed and never pushed**, which is the part not to relax later. `push_gate` is the only thing in the runner allowed to authorise a push, and this work has passed nothing - not the local gate, not the path denylist, not CI, not a review. A rescue that reached origin would be the one outcome worse than deleting it.

This applies to both hand-backs, because both lose work the same way. The dead-run guard is the one the incident hit. The pre-push hand-back is the other: `attempt_verdict` fails an attempt whose changes are still in the working tree, so a ticket can exhaust all three attempts while holding real work, and "none of the three passed" and "there is nothing here worth keeping" are different statements. Two harness cases pin them, and finding the second one took a correction worth recording: `dirty dirty dirty` does not exhaust the budget, because the second pass commits the stray file the first left and rewrites it identically, so the tree comes back clean and the ticket converges on attempt 2.

The plan's original "no WIP branch left dangling" is narrowed by this, deliberately. It now means no branch is left for a run that had nothing on it - which is still every ordinary failure.
...
Reuse the existing self-hosted ntfy setup and its severity conventions. PR-ready is low-priority/informational; stuck needs a human's eyes but not their phone buzzing, so it lands at the same silent `low` level and is told apart by title and tag; a usage-cap threshold alert (e.g. 80% of the weekly or monthly Go cap) was originally meant to sit alongside the other two, but is split out to #221, which reads OpenCode's own usage API (see Built).
...
### Built, 2026-09-09 (#176, PR #220)

Landed as one `notify` verb and two call sites in the runner (`modules/services/afk-agent.nix`), pinned by `checks/afk-agent-runner.nix`:

- **They publish straight to ntfy, not through Alertmanager.** These are pipeline events only the runner knows about - a pull request handed over, a ticket handed back - not metric states for Prometheus to scrape, so wrapping them in an exporter and a recording rule would be machinery in front of a message that is already in hand. The route is the self-hosted server on loopback (ntfy.nix owns the port; the bridge's public base URL would make the runner's own notifications depend on the tunnel standing up, which is what a tunnel alert would be competing with), the same topic the push lane publishes to, and the same access token the bridges publish with (`monitoring/ntfy/alerts-token`, added to the runner's credential set - it is already read by both hosts' bridges, so no new secret exists to mint or rotate). The token reaches curl as a header file under the run's scratch directory rather than as an argument, on the same reasoning that puts the App key on a path: a command line is readable through /proc.
- **Both arrive silent, at `low`, the lane's informational level.** Nothing is wrong with a PR ready for review, and a stuck ticket needs a human to read it rather than be woken by it at 03:00 - so neither buzzes, and `low` is the level warnings get. Distinguishability is title and tag on top of that: `AFK agent: PR ready for review (#n)` with `white_check_mark`, `AFK agent stuck on #n` with `octagonal_sign`. Publishing to the same topic rather than a new one was the smallest change and needs no new subscription; a dedicated topic is a one-line change if it ever wants separate muting.
- **The stuck notification is one helper, shared by both stuck paths.** The hand-back and the dead-run guard each assemble the same title, tag and priority, differing only in the prose above the ticket link - so `notify_stuck` carries the `low octagonal_sign` call and the `Ticket: https://…` line, and each caller supplies its own reason and status.
- **Best-effort, everywhere.** A notification that cannot be sent is one line in the unit's journal, and nothing downstream of it changes. The one thing that _does_ page through the existing stack is a runner that dies outright: `die` leaves the unit failed, which `SystemdUnitFailed` and the journal-tail enrichment already report.
- **The harness asserts the POST, not just the event.** `curl` left the runner's `runtimeInputs` (in production it is on the unit's PATH through the toolchain either way), so `checks/afk-agent-runner.nix` records every notification the way it records `gh` calls: a clean run publishes exactly one, at `low`, carrying the pull request URL; a handed-back ticket publishes exactly one, at `low`, named for its ticket; and a publish that fails changes nothing about the hand-back.
...
The third condition - OpenCode Go usage approaching a cap - is **split out to #221**, not built here. A first pass recorded the runner's own spend in a local ledger and alerted on a threshold crossing, but that answered the wrong question: it could only see this runner's sessions on this host, while the cap that matters is the account's, and OpenCode exposes it directly (`GET https://opencode.ai/zen/go/v1/usage`). #221 reads that API as the source of truth, alerts on daily/weekly/monthly usage, and designs its own dedup - the API is state, not a crossing, so the "notify on crossing, not on state" logic this first pass needed does not transfer. #176's "tested by hand once each" was met for the two notifications here (sent to the `alerts` subscription and confirmed on the phone); the usage alert is #221's to test.

The Done-when's "tested by hand once each" remains the human half: the two notifications go to the `alerts` subscription Cory already has, and the first real PR and the first stuck ticket are the live test of the phone's rendering.
...
### Built, across #171-#176 rather than as an item of its own

This item never got a commit of its own, which is what the approach above predicted: every secret the pipeline needs arrived in `secrets/` alongside the code that reads it, following the existing convention, and the convention needed no new design. Recorded here so that the table stops claiming work nobody is going to do.

**Four entries, not the two the problem statement named.** `opencode/api-key` and `opencode/username` are in `secrets/homelab01.yaml`, parked there before the switch was ever on (`secrets/README.md` says why a switched-off service still keeps its secret in its host's file). `AFK_AGENT_TOKEN` is gone: #200 replaced the PAT with a GitHub App, so what the host now carries is `gh-ci/afk-agent-app-private-key` - the App's private key, in the same file - and the runner mints an installation token per run from it. The fourth arrived with item 9: the notifications publish with `monitoring/ntfy/alerts-token`, which is in `secrets/shared.yaml` because the push lane already owned it and a second copy would be a value that drifts.

**Nothing here is a build log or a committed file.** All four reach the unit as systemd credentials - one `credentials` attribute set in the module drives the `sops.secrets` declarations, the `LoadCredential` list and the preflight together, so a set that drifts between them fails at evaluation rather than at 04:00 on the host. Inside the run they stay off argv: the ntfy token is written to a 0600 header file that `curl` reads with `-H @`, and the App token reaches `gh` through the environment, for the same reason - `/proc` makes a command line readable by every process on the box.

**Proven three ways.** `checks/secrets.nix` asserts every name a host declares exists in the file that host would read it from, which is what makes the four above more than an intention; `checks/stub-secrets.nix` lets `checks/afk-agent.nix` drive the real unit without real values, and asserts the stub's contents never reach the journal; and the live half is the runs items 5 and 14 already record - #195 carried a real ticket under the unit's own `ExecStart` and environment, and #200's App authenticated and committed as itself.
