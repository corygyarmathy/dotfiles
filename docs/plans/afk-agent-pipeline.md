...
That is a fact about the review rather than about the stage's wiring, so no amount of further building here addresses it - but "unmeetable" would be the wrong word, and an earlier draft of this note used it. The model experiment that sat between "not met" and "cannot be met" has since been run (see "Measured again"), and it did not lift the criterion: a stronger model saw the defect in 2 of 3 runs and gated on it in none. What it did do is move the open question from capability to rubric, which is a cheaper thing to test. Until that is tested, this item stays blocked on it, not finished.
