...
### The hue ring, 2026-09-01

"A new shelf in the vault is one line of CSS for all three" was written above as a virtue and is the defect. Which shelves exist is a fact about the vault this morning; a stylesheet that has to be edited when a folder is added is a stylesheet that knows something it has no business knowing, and until it is edited every note on the new shelf is grey. `.topic-slip-box` and `.topic-lighting` were the only two rules, so `essays` — a shelf the fixture has had all along — was not a design decision to leave hue-less, as the paragraph above claims. It was a folder nobody had written a line for.

It is now a ring of **eight numbered slots** in the stylesheet and a `hue-N` class per note, with `assign_topic_hues` in `publish-filter.py` deciding which shelf takes which. A new shelf gets a colour on the next build and nothing is edited.

**The slot comes from a hash of the shelf's own name**, not from its position in a sorted list, and that is the design rather than an implementation detail. Sorting is simpler and cannot collide, but it makes every shelf's colour a function of every _other_ shelf: publishing a folder called `essays` would renumber everything after it, and a reader who has learned that orange means lighting would find it means something else. Hashed, a shelf's hue is fixed by its name and survives every future folder. Collisions are the price and they are bounded — the second name reached takes the next free slot — so the one case where a new shelf recolours an existing one is when it both collides with it _and_ sorts before it. Past eight shelves the slots are shared, which is the honest thing for a ring of eight to do.

The ring's **order** is arbitrary, since slots go out by hash, so it was chosen: `slip-box` hashes to slot 4 and `lighting` to slot 2, and green and orange sit there, so the two shelves this vault publishes keep the colours they have always had.

**Kanagawa's yellows are excluded, and that is the only exclusion with a reason rather than a preference.** carpYellow and lotusYellow2 are within a few percent of the bonsai trunk's boatYellow1: a shelf drawn in one of them puts foliage and wood in a single colour and the tree stops having a structure to read. It was tried, rendered, and is why the ring is eight hues out of a possible nine. Everything left is already spoken for elsewhere — the callouts have six of them, links have crystalBlue and the site's name has sakuraPink — and that reuse is accepted rather than worked around: a shelf's hue and a callout's hue never appear in the same place.

**Two frontmatter keys, not one.** `topic` is the shelf's name — fixed by the vault, and what the markup can be read and asserted against — and `hue` is the slot, which depends on what other shelves exist. Only the second carries colour. `hue` is a **string**: Go templates treat the integer zero as false, so `{{ with .Params.hue }}` on a number would silently drop the class for whichever shelf landed on slot 0, which is one shelf grey and only if that slot is occupied.

The four templates that wrote the class inline now share `_partials/topic-class.html`, which **returns** the attribute rather than printing it. An attribute _name_ is a context Go's `html/template` will not admit an ordinary partial's output into — it substitutes `ZgotmplZ` and the class vanishes from every page with no build error at all. Marking the value in the partial means no caller can forget to, and the failure mode is now asserted directly, because it renders a perfectly valid page with all of its colour missing.
...
**So the two variables went onto two axes**, which is the move item 15 had already made when it put maturity on shape and topic on hue, for the same reason: when one channel is asked to carry two things, the vault decides which one loses. The map is now three facts on three channels — the NAME is a line of text, always legible and never behind a hover; the WIDTH of a rule beneath it is that section's share of the note; the FILL of that rule, left to right, is how much of it has been read. A 12:1 ratio is 20px against 240px, which is legible at a glance, and widths have room that a column of text does not. It is a bar chart lying on its side that happens also to be the table of contents, and the silhouette down the margin is the shape of the document. The template does no arithmetic: `--w` is each section's word count and `--widest` the largest on the page, and the stylesheet divides the two. Normalising per page rather than per site is deliberate — the map answers "how is THIS note apportioned", and normalising against the site's longest section would draw every short note as a row of stubs.

**The progress is the whole note's, not the section's, and that is what made it complete.** Sections above the one being read are filled, the one being read fills as the reader moves through it, and the ones below are empty — so the strip is a progress bar for the note as well as a map of it. The reported fault was that it never finished: the first cut ran the last section from its heading to the foot of the `<article>`, and the article's foot sits above the footer, so the reading line at the viewport's top can never reach it and the last bar stopped short no matter how far the page was scrolled. Clamping that end to the furthest the page can actually be scrolled fixes it by construction — at the bottom of the page the reading line _is_ that value, so the fraction is exactly one. Measured in a browser: five pixels short of the bottom the last bar reads 0.989, and at the bottom every bar reads 1.000.

**The scroll-spy was then rewritten a second time, around a single reading line, and that is the part of this item worth reading twice.** Three faults came back from looking at it on real notes, and all three were the same cause: there were _two_ reading lines. An `IntersectionObserver` band across the top fifth of the viewport decided WHICH section was current, and the viewport's top edge decided HOW FAR THROUGH it the reader was — two lines a fifth of a screen apart.

- The progress appeared to **start late**, because a section became current a fifth of a screen before its bar began to fill.
- It **jumped to full at the end**, because the bar completed when the next heading reached the band rather than when it reached the top: on a 347-word section that is 64% of the bar arriving at once, which is why it was worst on the short ones and still visible on the long ones.
- And it **flickered when scrolling up**, because several headings sit in the band together and the observer batches their entries, so the choice of "topmost heading in the band" flipped between frames. Scrolling down it never did, because headings leave the band from the top in order.

One line fixes all three by construction. A section becomes current at the instant its heading crosses the line, which is the same instant the previous section's bar reaches exactly full; the line is monotonic in the scroll position, so there is nothing for the highlight to flicker between; and the observer is gone with the band it existed to define, replaced by a `ResizeObserver` on the article for the one thing the observer was still doing usefully — noticing that an image or a font has changed every offset without a scroll or a resize.

**And the line has to reach the foot of the article, which a line at a fixed offset cannot.** The page stops scrolling with a whole screen still below the line, so every heading in that last screenful never becomes current at all. This is not a corner case, it is most short notes: of the three sectioned notes in the vault, two had their last heading past the furthest the line could reach — Lighting Colour Selection's at 3729 against a 3011 limit, Effective Lighting's at 2910 against 2538 — and on both the final section never highlighted and its bar never filled. Riverview was the only one that worked, and only because it is long enough that its last heading falls inside the reachable range, which is exactly why looking at one long note was not enough. Over the last screenful of scrolling the line now accelerates to land on the foot of the article at the exact scroll limit — the reader's attention moving down the viewport because the page can no longer move under it — and the term is zero before that stretch, so the rest of the document reads one-to-one.

**One thing about the script survives from the first rewrite.** The animation-frame coalescing that was tried was withdrawn: the browser already delivers at most one scroll event per frame, so it bought nothing, and the "a frame is already queued" flag it needs is only ever cleared by the frame running — a hidden tab suspends animation frames, so a page scrolled in the background came back with the flag latched and the map dead for the rest of its life. A plain call from the scroll handler has no state to latch.

**The maturity mark is item 15's, not a second one.** The first cut of this item predated item 15 landing and grew its own sprite in `_partials/sprouts.html`, coloured `--accent`. That was wrong twice: a duplicate of a sprite the site already ships, and a hue that reports nothing — the mark's colour is supposed to be the note's topic, which is what the /notes/ index and every internal link already do. The margin now calls `mark-icon.html` and puts the `topic-*` class on the value, so the shape is the note's maturity and the hue is its shelf, decided in the two places item 15 decided them; `sprouts.html` is deleted. And the dates became two facts with two labels: `PUBLISHED / 31 Aug 2026 · updated 1 Sep 2026` is a label that names half of its own value, and it was the only row in the column breaking the one-label-one-value rule the rest of the margin keeps.

The `--reading` role variable came out of this. "You are here" already had its own hue per the 2026-08-27 decision, written as a `light-dark()` literal in one rule; the bar's fill needs the same colour, and a hue with two call sites is a role rather than a value. It is declared beside `--accent` in both themes, carrying the note that Lotus can keep the accent here and Wave cannot.

The VM check pins the properties: every staged note carries `sections`/`word_count`/`reading_time`; a note with neither sections nor backlinks still renders the margin and its facts (on-money, whose hand-written evergreen override is asserted on the page as `#mark-evergreen`, in its shelf's hue); the map renders only past the three-section threshold, with every entry's `href` landing on the id the filter computed and distinct word counts (the fixture's sections are deliberately different lengths so the proportionality cannot silently become a list); and the dateline swap is pinned in the stylesheet rather than assumed in the markup. Two assertions go further than the rest, and they are the two that would have caught what shipped first: a chromium probe measures each bar's rendered width against its word count, so a map that renders as equal bars fails the gate; and the same probe measures every section NAME's height, opacity and visibility, so a map that hides its names behind a hover fails it too — the three ways a label disappears while its markup stays in the page and passes a grep. A third sweeps the whole scrollable range with synthetic scroll events, driving the page's own handler rather than a reimplementation of it, and asserts properties of the _sequence_: every section becomes current at some point, the mark never travels back up the list while the page travels down, every bar is full at the foot of the page, and the remainder on a bar when the next section takes over is within one sweep step of zero. Two of those were checked by putting the bug back and watching them fail, because an assertion that cannot fail is worse than none: disabling the end-of-document term fails with `sections that never become current: [3]`, and reintroducing the two-line split fails with `section 0 jumped 79px to full when section 1 took over`. The monotonicity assertion is a guard rather than a reproduction — the geometric line cannot go backwards, which is the point of it — and it is kept so that a future rule which can will be caught.

**The facts swap whole between the two layouts, and the first cut swapped half of them.** The dates moved up to the dateline under the title at the narrow width while the reading time and the maturity stayed in the margin — which at that width is _below_ the essay. So a note's own facts arrived split in two, half above the prose and half after it, and the half that came after was reached only by a reader who had already finished reading. All four now sit on the dateline there and in the column here, one copy visible at a time.

That line is assembled from whichever facts a note has, and the separators are drawn by CSS rather than emitted by the template. A template that decides its own separators has to know which fact is first, and every fact except the date is optional; `span:not(:last-child)::after` knows it without being told, and a leading "·" becomes unwriteable rather than merely avoided. It hangs off the fact _before_ it rather than the one after, which matters only when the line wraps — and on a phone it does, because four facts are wider than the viewport, so putting it on the following fact opens the wrapped line with a stray "·" under the title.

`dateline.html` is now called two ways: `page.html` asks for the whole line, and the /notes/ index asks for the dates alone, because its rows already carry a maturity mark on each title and a reading time on every one of twenty-one rows is noise. One partial told to say less, rather than two partials that will drift.

**And hiding the facts at the narrow width left an empty box.** With the facts on the dateline and the map suppressed (a contents list of the page you are already on, placed below that page, is navigation nobody can use), the backlinks are all the column has left down there — so a note that nothing cites rendered an empty `<aside>` whose top border drew a rule across the page above the footer, separating the essay from nothing at all. The template adds a class when the note has no backlinks and the stylesheet hides it below the breakpoint, which is the one place item 17's "the column is unconditional" gives ground: it is unconditional where it has something to say.

One detail is worth recording for whoever reads this next. The reading time is one minute per two hundred words, which says of a twenty-one-note garden mostly "a minute" — the same honest-number-wins reasoning item 9 recorded. And the margin's facts duplicate the article's in the HTML, one copy per layout, which is a deliberate trade: the alternative (a single copy repositioned by CSS) cannot reach across the grid from the article into the margin, and two copies hidden and shown is the smallest thing that works.

**Still open.** The rendering fixture's long note has twelve sections of 32 to 65 words, so its bars are all within a factor of two and the map's proportionality barely shows on a fixture screenshot — the property is gated by the check's own fixture, but item 1's premise is that a stylesheet change should be judgeable by looking, and this one is better judged against the real vault than against the fixture. Giving `A Long Note.md` sections of deliberately uneven length would fix that.
...
The taste pass tuned four numbers and left the growth model alone, and two of the three faults it was asked about came back. Stated as they were reported: the tree only looks like a bonsai on _some_ seeds, so the shipped one had to be chosen by hand; and past about fifty notes the canopy becomes a blob that only grows. Both are properties of the model, not of its constants, so this session replaced the model.

**The order was wrong, and that is the whole diagnosis.** Under `cbonsai`'s model the branches are walked at random and a pad is dropped wherever each one stops, so the union of those pads _is_ the silhouette — every pad placement is a chance to spoil the shape, and with seven pads there are seven chances. No constant fixes that; it is what the model is. So the order is inverted: **the crown's outline is computed first and nothing downstream may change it.** A flat-bottomed dome, lobed by two low-frequency harmonics of the angle rather than by per-cell noise, which reads as foliage where independent jitter reads as a rendering fault. The trunk is a designed S-curve with a decaying amplitude, replacing a random walk that needed a persistent lean _and_ a rein on how far it could wander to approximate the same shape. What the pads decide now is only where the sky _inside_ the outline falls: a cell roughly equidistant from two pad centres is dropped, and those channels are what separate one plate of foliage from the next.

444 trees were rendered across sizes 1 to 500 and twelve seeds. The shape no longer depends on the seed, which means `DEFAULT_SEED` has stopped being load-bearing — it was the only thing standing between the site and a bad tree.

The terminal loop the taste pass named is what made this affordable a second time, and its flags have moved with the model: `SPREAD` is gone with the branch-count rule it belonged to, and `--seeds N` replaces it, drawing N seeds in a row because judging consistency is now the thing that loop is for. `python3 modules/services/digital-garden/bonsai.py --notes 120 --seeds 6` is the command the paragraph above is reaching for.

**The blob was an arithmetic problem and had a arithmetic answer.** Every note took six to twenty cells whatever the vault held, so the canopy's _area_ grew linearly with the garden: about 215 cells at nineteen notes, 1,430 at a hundred and twenty, 2,400 at two hundred — by which point there was no sky left to lose. A canopy is a silhouette, not a sum. The area is now budgeted at `30 × notes ** 0.6`: nineteen notes draw about 245 cells and two hundred about 920, so four times the notes buys less than four times the foliage. The pad count is capped at seven for the same reason — a trained bonsai carries a handful of pads however old it is, and past eleven the channels between them outnumbered the plates and the crown came apart into islands.

**"Too flat" was two problems wearing one name, and both fixes are PyBonsai's.** Maturity moves from one fixed glyph to a density-ordered _set_ — `.,'` for a seedling, `*^v+` for a sapling, `&%#@` for an evergreen — with each cell picked out of it. The sets are as far apart as the three single characters were, so the stage still reads at a glance; what varies inside a set is texture and carries no meaning. And every cell now carries which of three lightness tiers it falls in, from one light source over the whole crown. Shading each pad from its own centre was tried and gives seven separately lit blobs; one source is what makes the mass read as a form. The trunk's lit edge and shadowed edge are the same mechanism and are what make it a cylinder rather than a stroke.

The stylesheet does that with one indirection worth recording, because it is not obvious and the failure is silent: every cell declares what it is _made of_ as `--base` and only three rules set a colour. `currentColor` cannot do the job — in a `color` declaration it means the value inherited from the parent, which here is the `<pre>`'s brown, so a leaf shaded from it comes out a lighter brown rather than a lighter green.

**Foliage may now cover wood.** Refusing every overwrite — which is what "a note must never hide another" had been generalised into — left a bare trunk running up through the middle of the crown with a channel of sky either side of it. A canopy covers the branch holding it up. Nothing covers foliage, so the rule that mattered is intact, and the note-count assertion is unchanged and still fires on nothing.

**The soil line became a pot**, and it is the one addition here that reports nothing about any note. Neither did the row of soil it replaces, which was already on exactly that footing: it earns its place because this is a picture of a _bonsai_, and a tree without a pot is just a tree. It is the cheapest thing on the page that makes the object read as what it is meant to be, and it was the change that moved the reading furthest per line of code.

**One idea was built, looked at, and thrown away.** Grouping each shelf's notes onto a pad of its own would have made every plate of foliage one shelf of the vault — the tree saying something it could not say before, and a natural pairing with the hue ring in item 15. Rendered at five shelves it draws hard-edged blocks of colour and the tree reads as a chart of itself. Hues have to interleave at the scale of a note's clump, which is the scale foliage varies at, so the notes stay spread and only each note's own cells are contiguous. That is the one decision in this session that reasoning got wrong and looking got right, which is item 1 earning its place again.

### Growing, and a tree per vault, 2026-09-03

Two faults reported from using the page, and they turn out to share a cause: both are places where the tree stopped being a function of the garden and became a fixed drawing that the garden was allowed to nudge.

**The tree did not grow, it was wiped on.** The reveal walked the cells in the order `bonsai.py` happened to _draw_ them and paced itself at eight characters a frame. Three things about that are wrong about a tree, and all three were visible: the trunk is about fifty cells of three hundred, so at a fixed rate it arrived in a tenth of a second as one object rather than as something that rose; the foliage came in as horizontal scan lines, because that is the order the crown's cells are enumerated in; and the pot filled in last, when a pot is the one object in the picture that was never grown.

The order a tree is _painted_ in is a fact about painting — foliage is drawn after wood so that it can cover the branch holding it up — and it was never going to be the order a tree _grows_ in. So growth is now computed, as a number from 0 to 1 per cell, from the tree's own geometry: the pot is set down, the trunk rises out of it, **each branch leaves the trunk at the moment the trunk passes its height**, and each pad opens outward from the end of the branch that carries it. That last pair is what makes it read as growth rather than as a sequence of stages — the lower pads are in leaf while the apex is still rising, which is what a tree looks like and what a stage list cannot look like.

The page then walks that timeline **against the clock rather than against a count**, which is the other half of the fix. Counting is what made the trunk instant; against the clock the trunk gets the half of the animation the generator gave it, a two-hundred-note vault grows in the same 2.6 seconds as this one, and a machine that cannot keep up drops cells in bigger steps instead of taking longer. A reader who opens the site in a background tab now gets the finished tree, because the time passed while they were away — under the old count-based loop they came back to bare soil and watched it grow as if they had just arrived.

Each character fades up over 0.35s rather than appearing, and each leaf arrives a shade lighter than it settles at, over 0.9s. The pale arrival is the one that earns its line: new growth on a real tree is lighter than the wood under it, and at this resolution — one character per leaf — a pad that opens pale and darkens into the canopy is the only cue that says the crown has depth rather than being a stencil that filled in. Both are transitions off the same `.off` class the script was already toggling, and both are scoped to a `.growing` class taken off at the end, so the hover highlight stays instant.

**The seed is now a hash of the published notes, and the fixed seed was a wrong answer to a real constraint.** The constraint is that the same vault must draw the same tree: the builder skips a rebuild when the staging tree hashes the same as last time, so a tree that reseeded itself every hour would defeat that gate and rewrite the site for nothing. A constant satisfies that. So does a hash of the garden — and a hash also does the thing a constant cannot, which is to make the tree a _fingerprint_ of the vault rather than a fixed picture the vault is allowed to widen. Write a note and you get a different tree: a new trunk, new lobes on the crown, new pads. Change nothing and it is the same tree it was this morning, forever.

This is only affordable because of the rebuild above. Under the old model the shape depended on the seed and only some seeds drew a bonsai, so a seed that moved with the vault would have been a coin flip on every note written; the shape is now designed and the seed varies the movement _within_ it, which is what made "let it reseed" a small change rather than a gamble. `DEFAULT_SEED` stopping being load-bearing was recorded above as an observation — it is the thing this session spent.

Word counts are in the hash, which means fixing a typo redraws the whole tree. That is the intended reading of "a picture of the garden as it is", and it is one line in `seed_from` if it ever grates.

`--seeds N` on the command line changed meaning to match: it now varies the _garden_ and lets each one seed itself, because what varies on the site is the vault and a loop that held the notes still and turned the seed would be judging trees the site can never draw.

**The trunk gained a nebari** — the root flare where it meets the soil, `__/|||\__`, in wood rather than in the soil's grey. A bonsai is judged on its nebari before anything else and the reason survives to this resolution: a trunk that meets the ground as a straight column of three characters reads as a pole stuck in a pot. Two characters of spread either side is the whole difference, and it is the cheapest thing added to this picture since the pot.

It cost one line elsewhere, and the line is the interesting part. The trunk is stepped at about four tenths of a row, so **two** steps land on the soil row, and the second one carries the lean — so a trunk with any movement in it met the ground four cells wide with a slash hanging off one side, and the flare below then met that slash point-to-point and read as a notch cut into the foot. The soil row now takes the first step and no other: a trunk does not lean where it enters the ground. The flare is also measured off the wood that was actually drawn rather than off `trunk_x(0)`, because the only reliable answer to "where does the wood end" is to look.

**An empty garden took the build down, and had done since the pot was added.** `publish-filter.py` writes the tree unconditionally so that unpublishing everything leaves an empty plate rather than last build's tree — which is precisely the path that reaches a crown with a radius of zero, and every radius below it is a divisor. Guarded at the top of `grow`, which is where the count is already known.

### Three left on the table, 2026-09-03

Named here rather than done, because each is a taste question and this session was two bug fixes wearing a feature's clothes. In the order they are worth taking.

**New growth belongs at the tips, and the tree does not know it yet.** A note's position inside its pad is arbitrary today — the pads are dealt by word count so that the long notes do not all land on one plate, and inside a pad each note gets a seed point at an angle off the centre. Deal by _age_ or by the ledger's revision count instead and the picture gains an axis it does not have: old, heavily-revised notes low and close to the trunk, new notes out at the extremities. That is how a tree actually grows, it is a fact the vault already records, and it is the one idea on this list that would make the tree say something about the garden's _depth_ rather than about its size and its spread. It is also the one that touches the most code — the pad dealing and the per-note seed points in `grow` — which is why it is a session and not an afternoon.

**The trunk is thin for a bonsai, and the taper is anchored to the wrong thing.** `thick` steps 3 → 2 → 1 at fixed fractions of the trunk's total height (0.16 and 0.55), so how much of the _visible_ trunk is at full width depends on how tall the tree happens to be rather than on where the crown starts. Anchoring it to `clear` and `crown_y` instead was tried on 2026-09-03 and barely moved at nineteen notes — the real levers are a longer `clear` and holding three cells further up, which is a change to proportions that item 18b tuned by looking. So: worth doing, worth doing _in_ a taste pass, not as a drive-by. A thick trunk under a shallow crown is most of what separates a bonsai from a sapling in a pot.

**The crown's fringe reads as dust.** Around the lower-left edge, where the seedlings' `.,'` set lands on the outline, the tree scatters into single characters that read as noise rather than as foliage. The pruning pass already exists and already removes cells with fewer than two orthogonal neighbours, twice; raising that to three for one of the passes would tighten the edge. It is a two-character change and it is not obviously right — the threshold eats into the silhouette this whole file exists to protect, and the fringe is also what stops the crown looking stamped. It needs the many-trees loop and a decision, not an edit.

### The three taken, and a fourth, 2026-09-04

All three of the ideas above were taken. Two shipped, one is declined on measurements — and the fourth item, which was not on the list, is the one that turned out to explain the others.

**The reveal was too smooth, and the fade was hiding a badly-shaped timeline.** Reported as _"more reveal than growth — it should look like it is being generated in real time, not revealed when the page loads"_. The obvious reading is that the 0.35s per-character fade is the fault, and it is, but only half of it. At about one character every nine milliseconds a 0.35s fade puts **forty cells on screen part-drawn at any instant**, and forty part-drawn cells is a soft-edged front sweeping over a finished picture. That is a wipe. It is what the word "reveal" was pointing at.

The other half only showed up on being counted. Bucketed into twentieths of the run, the old timeline arrived as **a lump, a stall and an avalanche**: fifty cells of pot and trunk base in the first tenth, then about a quarter of the entire animation in which a one-cell-wide trunk was the only thing being drawn, then a third of the tree in a fifth of the time as every pad opened at once. The fade was wide enough to smear all three into something that read as smooth. Take the fade away and you get a stutter, a wait and a flash.

So the fix is mostly in the generator, and the page's half is a deletion. **Nothing transitions any more**: a character arrives at full strength in one frame. The smoothness is bought back from the rate instead of from the blur — three or four hard-edged characters land per frame at sixty frames a second, and the cumulative effect is smooth while every individual character snaps. The tick went from 32ms to 16ms for the same reason: with nothing interpolating between frames, the frame rate _is_ the smoothness.

**The stall and the avalanche had one cause, and it was not a constant that needed tuning.** A bonsai's trunk is bare low down — that is what `clear` is for — so every branch leaves the trunk in its upper half, and "a branch leaves at the moment the trunk passes its height" put all five pads within a fifth of the run of each other. There is genuinely nothing to draw while the bare trunk rises, and then there is everything to draw at once. No amount of moving `TRUNK_TO` and `LEAF_SPAN` fixes that; a sweep of thirty combinations got the worst slice from 1.51 to 1.11 times off a flat rate and no further.

The crown now gets **a span of its own that the pads divide by cell count** — a pad carrying eighty cells gets four times the clock of one carrying twenty, so every plate fills at the same rate and the crown fills at one rate. The causality survives as an _order_ rather than as an arrival time: a pad still cannot leaf before the branch carrying it has reached out, and the pads still leaf in the order the trunk reached them, so the lower crown is still in leaf while the apex is still rising. Measured over four vault sizes, the stalled slices went from six consecutive to **none at all** at 19, 45 and 120 notes.

**One cue survives the deletion, as a step rather than as a fade.** New growth on a real tree is lighter than the wood under it, and that was worth keeping — but as two discrete states, not an interpolation. The script runs **two cursors**: one snaps cells on, a second follows 170ms behind and takes a pale tint back off. What moves over the tree is a band of fresh growth a few frames deep, snapping in and snapping down. Both read the same clock, so the band's depth is fixed in milliseconds however fast the machine is drawing.

`bonsai.py --rate` prints the arrival histogram, and it is in the repository for the same reason `--seeds` is: the timeline's shape is an emergent property of the crown's geometry and changes whenever the geometry does, so it is a thing to look at before touching either.

**New growth at the tips: done, on revisions rather than on age.** The ledger settles which fact. Every note in the vault except one carries `published: 2026-08-31`, the day the ledger was seeded, so a pure age axis has almost no spread to give today; `revisions`, seeded from the vault's own git history, spreads nought to eight right now. So revisions leads and the date breaks its ties, which sharpens the axis by itself as the garden ages and notes stop sharing a birthday. Ranked rather than scaled, so the axis is fully used whatever the distribution behind it looks like.
...
Shipped, the axis measures **rho +0.74** overall, from +0.57 at eight notes to +0.82 at forty-five. It is weakest on the smallest gardens, which is honest — a tree with eight notes on three plates has very little inside to be inside of. It costs a little clump tightness at small sizes: a note's mean radius goes from 1.34 to 1.63 cells at nineteen notes, because a note now occupies a band at a depth rather than a disc. A clump three cells across is still a clump to point at, which is the property that matters.

**The first version of that number was +0.85 and it was wrong**, from measuring one garden per vault size rather than six. It is recorded because the error is the interesting kind: it was not a bug, the code did exactly what it does now, and a single sample of a noisy statistic reported it a tenth better than it is. Six gardens per size is the smallest sample that stopped the number moving between runs.

**The trunk: the taper was anchored to the wrong thing, and so was the diagnosis.** The idea above names two levers, a longer `clear` and holding three cells further up. Both were built. Neither was the fault. The fault was one line of glyph selection: **the middle column took the lean's own diagonal**, so a leaning trunk drew `\\\|` — every column the same stroke — and three cells of wood read as one hatched line rather than as a column that has shifted. The lean is already carried by the x position the whole group is drawn at. The edges should show it; the core should stay upright.
...
**The crown's fringe: declined, and the premise did not hold.** Counted over forty-eight trees at four vault sizes, **0.3% of crown cells have fewer than two orthogonal neighbours** — there is no dust to prune, the two existing passes already removed it. The crown's edge cells carry _more_ ink than its interior (2.41 against 2.33 on a three-point scale over the glyph sets), not less. On the real vault: 0.4% specks on the tree this replaces and 0.0% on the one it draws.

The proposed change makes it worse in a way worth recording, because it is not obvious. `(2, 3)` costs 14.5% of the canopy and takes specks from 0.3% **up** to 2.1%: a pass at three removes cells that have two neighbours and orphans whatever was leaning on them, with no low pass left to clean up after it. Only `(3, 2)` — erode hard, then tidy — improves on today at all, and it buys 0.3% down to 0.1% for 15.6% of the canopy. Rendered at three seeds and three vault sizes, what that 15.6% costs is holes in the crown's _body_ and channels between the pads wide enough to come apart: the silhouette the whole file exists to protect, eaten from the inside.

**And the fringe now means something, which is the argument that closes it.** Dealing the notes inward-to-outward by revision count sorts the glyph sets along with them, because a note nobody has gone back to is usually a seedling and a seedling's set is the faintest. On the vault, the crown used to carry the same weight of ink in its outer half as in its inner half — 1.97 against 1.97 — and now carries **1.48 outside against 2.18 inside**. That gradient _is_ the wispy edge somebody looked at, and it is now the picture reporting that the tips are this year's growth. Pruning it away would delete the signal the item above was added to carry. Left as a named `PRUNE` constant, because the idea is a natural one to have twice and that is the cheapest way to try it again.

**Two assertions, and a lesson about writing them.** The VM check now pins that the growth timeline runs 0 to 1000 and that its order is botanical: the pot is set down first, the trunk rises out of it, the crown is the last thing to arrive. Stated as exact bounds and not as a rate, because a rate assertion on a four-note fixture measures noise — `--rate` is the taste loop and not a gate. This fails silently if it fails at all: a tree whose timeline has collapsed still renders, still carries every note and still passes every other assertion in that subtest.

The lesson is that `re.findall` returns strings, so the first two versions of that assertion compared timeline positions **lexicographically** — `"106" < "1000"` is False — and each produced a confident, wrong diagnosis of working code, one of which was written up as a design property before the arithmetic was checked. The measurement lying is the same failure as the code lying, and it is harder to notice because a measurement has no output to look at. A parallel one: the first pass of the fringe analysis dropped every evergreen cell, because `html.escape` turns `&` into `&amp;` and a one-character regex silently skipped them — which reported 20% specks on a crown that has none.
...
`layouts/home.html` puts the name, the colophon and the tree in one two-column grid, `_partials/colophon.html` counts the garden, and `baseof.html` drops the small masthead title on the home page and nowhere else — a site title in two sizes on one page is two mastheads, and the large one is the composition. What is left of the masthead there is its two tools, which is all it was ever carrying besides the title.

**The composition is the one block on this site wider than the measure, and the width is not a new number.** It is capped at `--measure + --gutter + --margin-col`, 58rem: the width the prose gains when the wide grid gives it a rail. That was forced rather than chosen. The name wants about 480px at `4.25rem` and the tree is about 384px at the size item 18b left it, so the pair does not fit a 40rem measure at any size worth having — the only alternative was to shrink the tree back to the sketch the taste pass had just enlarged it from, which would have been this item undoing the one before it. The tools take the same cap, so the two icons sit over the right edge of the tree rather than over the middle of it, and everything below the composition is the measure it has always been.

**The name wraps to two lines, and it is better wrapped.** The tree takes the width it grew to and the name takes the rest, so at eighteen notes _Cory Gyarmathy_ has about 536px and needs about 553px. Both were rendered before choosing: at `4rem` it fits on one line and leaves a thin band of type beside a tall tree with a hole under it, and at `4.25rem` the two lines make a block that stands against the canopy. So the plan's "roughly 4.25rem" survived being looked at, and the wrap is a consequence of the composition rather than a fault in it — a smaller garden gives the name a wider column and it sets on one line again, which is what the three-note fixture does.

**Item 18b's ceiling was right and its floor was not.** The taste pass left `font-size: clamp(0.5rem, 2vw, 1.25rem)` on the tree and predicted this item might want the scale again; at the top of the clamp it did not, and the number that needed moving was the one nobody had looked at. `2vw` on a 375px phone draws a thirty-two character tree about 150px wide — a smudge under a name three times its size, on the layout where the tree has the whole width to itself. At `3.2vw` it is about 215px and still cannot overflow: the plate is as wide as 45 characters there, which is a two-hundred-note vault. Nothing above 625px moves, because the 1.25rem ceiling is already reached there.

**Two things were settled by looking at widths this machine does not have.** The composition stacks at 60rem rather than at the 48rem the prototype used: between those two the name and the tree do fit side by side, and what they look like is a tablet with a word jammed against a tree and no margin either side. And the phone layout centres all three parts rather than left-aligning them, because a centred name over a centred tree is the same composition seen end-on, while a left-aligned name over a centred tree is neither.

### Anchored, 2026-09-01

Reported as "the header feels disconnected from the page, maybe because it is so much wider than the body". The width was a red herring and the measurement says so. At 1440px every interior page's masthead rule runs from 400px to 1040px — exactly the article column, so it begins and ends on the prose under it. The nameplate ran from 305px to 1135px. It shared a **centre** with the page and not a single **edge**, and a band that lines up with nothing reads as floating over the page rather than as part of it. Being wider was never the problem; having no edge in common with anything was.

**The header now begins on the article column's left edge and ends on the wide grid's right edge** — the only two vertical lines this layout has. The name sits directly above the first line of prose, and the rule closing the composition finishes on the page's own outer edge. Its width is 58rem again, which is the number the first cut of this item used and the taste pass replaced with `fit-content`; what was wrong then was not the cap but putting the two columns at its two ends, which spread them 156px further apart than anything in them needed. The cap is back and the columns are packed left, so the tree still sits beside the name rather than across the band from it.

Asserted in pixels, not by grep. A probe in the VM test measures the header's two edges against the article's and the grid's, because this is a geometry fault: invisible in the markup and obvious in a screenshot, which is the exact class of thing item 1 exists for.

**Anchoring costs width, and the cost has a breakpoint.** A centred header may use the whole page; one that starts at the prose's left edge gives up half the difference between the page and the measure before it begins. At 1000px that left about 300px for the name — narrower than _Gyarmathy_ is at that viewport's type size — and the title broke across two lines with a lone "y" on the second. So the anchoring starts at 72rem and the 60–72rem band stays centred: it shares only a centre with the page, which is the thing this change exists to fix, but a header that fits and is centred beats one that is anchored and broken.

**The hole in the left column is closed, and it was a trade made in the other item.** `align-items: stretch` with an auto margin on the colophon put the count level with the tree's caption, so the two columns finished together as well as starting together. What that produced was about a hundred pixels of slack in the _middle_ of a text block, between the links and the count, which reads as something missing rather than as space. Top-aligned, the slack falls at the foot of the words where it is invisible. The two small grey lines no longer sit on one line, and that is the cheaper of the two costs — the original reasoning was sound and was simply outvoted by what it looked like.

**Projects and Resume join GitHub**, and an entry in `footerLinks` whose URL is empty renders as muted text rather than as a link — in the header and in the footer both, since it is one list. That is what lets the section show its shape before every destination exists; the alternatives were two 404s shipped on purpose, or two stub pages held in the site and in the search index to stand in for pages nobody has written. A plain `<span>`: `aria-disabled` says an interactive control is unavailable, and this is not a control, it is a word.

**The fixture had no evergreen, and the colophon is what made that visible.** `Every Element` was filed as the fixture's evergreen when item 18 wrote it, but three notes cannot earn 5.00 on the model's own terms, so the tree drew no `&` and the new count line rendered _0 evergreen_ on a page whose own prose said otherwise. It now carries a hand-written `maturity: evergreen`, which is the honest fix — it is what the override exists for — and it makes the fixture exercise all three glyphs, all three counts and item 14's override in the loop that item 1 exists to provide.

### Three faults found by using it, 2026-09-01

All three came from the first person to move a pointer over the finished page rather than to look at a screenshot of it, which is the gap between item 1's loop and a page in use.

**Pointing at a long note dragged the whole tree sideways.** `.bonsai-plate` is `width: fit-content` and the caption inside it could be up to 34rem, so the moment a caption was longer than the tree the plate grew to the caption — and inside the composition's `auto` column, that shortened the name's column and moved the tree left by as much as 140px. It moved back when the pointer left. The fix is `width: 0; min-width: 100%` on the caption: a percentage minimum contributes nothing to intrinsic sizing, so the tree alone decides how wide the plate is and the caption wraps to the tree. The caption now reserves two lines rather than 2.6em of one, because wrapping to the tree is what it does. Reproduced without a pointer by forcing the idle string in `baseof.html` to the longest caption the vault can produce and rendering that, which is the way to check it again: the tree's pixels are then identical to the idle page's, and the rule below it does not move.

**The stacked tree was not a tree.** The narrow layout centred the composition with `text-align: center` on `.nameplate`, and `text-align` inherits — so every row of the `<pre>` was centred independently and the tree came out as a spindle with its rows shuffled sideways. The rows are only in register while they share a left edge. Centring now happens on the grid (which moves the block) and on the text column (which is text), and never on anything containing the tree.

**The three parts did not read as a header.** They read as an upper-left thing, an upper-right thing and a centred body, and the clearest symptom was that the two columns were different heights. Three changes, and none of them is the one the symptom suggests: the columns are `stretch` rather than `center`, so the band has one top edge and one bottom edge; the name and the colophon are pushed apart to those edges, so what closes the left column is the same kind of thing that closes the right — one short muted line each, on the same line as each other, which `--caption-line` exists to keep true; and the rule the masthead gave up on this page is drawn under the whole composition instead, at the composition's width. The rule is what does most of the work: it is the difference between three things above the prose and one header with the prose beneath it.

### Four more from the second look, 2026-09-01

The first round fixed what was broken; this round is the composition itself, and every one of these came from the page being used rather than from a screenshot of it.

**A wrapped caption had its descenders shaved off.** `overflow-x: auto` on `.bonsai-plate` forces `overflow-y` to `auto` as well, so the plate was a scroll container clipping at its own content box, and the last line of a two-line caption lost the bottoms of its `p`s and `g`s. The scrolling belongs on the tree, which is the thing that can be too wide; the caption was never the part that could overflow and should not have been inside the part that could. `overflow-x` moved to `.bonsai`, whose bottom padding is inside its own box, so the soil still clears the edge.

**The room held open for a second caption line was in the wrong place.** It sat below the caption, which put a line of empty space between the header and the rule that closes it, and left the caption off the line the colophon sits on. The caption is now anchored to the bottom of its two-line box: a second line grows _up_ into the space under the tree, which is where a picture wants space anyway, and nothing below moves either way. That also retired the `--caption-line` lift on the colophon — with the caption on the bottom line of its own box, both columns simply end at the band's bottom edge.

**The band was 156px wider than anything in it.** Pinning it to `--measure + --gutter + --margin-col` was a defensible number and the wrong one: the name and the tree sat at the two ends of a width neither had asked for, and read as a left-hand thing and a right-hand thing rather than as one header. It is now `width: fit-content` over a column wide enough to set the name and its sentence (`minmax(0, 24rem)`) plus however wide the tree grew — the only two measurements the page actually has — and the whole header comes out about 830px at eighteen notes instead of 928px, with a 3rem gutter instead of 156px of nothing.

**The tools came inside.** They were a separate masthead capped at the old band width, so once the composition was sized to its contents they no longer sat over anything. They are now a cell of the same grid, above the tree and right-aligned in its column, which is why they land exactly on the tree's right edge. `_partials/masthead-tools.html` holds the two buttons so that the interior masthead and the home page's composition render the same markup and the ids the scripts bind to exist exactly once per page.

**And the empty half of the left column got the site's own sentence.** `site.Params.description` — the line already serving `<meta name="description">`, the social card and the feed, resolved through the same `description.html` partial so the page cannot say something different from what it tells a search engine it says. It is one line of configuration rather than prose written for a composition, which is the same test everything else on this page has to pass: it reports something true, and it was already true before this item wanted it.

**The direction was reconsidered and kept.** The obvious alternative is the narrow layout used at every width — name, sentence, count and tree centred in one column — and it was rendered at 1440px to be sure. It has no empty space anywhere, which is its whole appeal, and it costs about 180px of height before the first heading and gives up the one thing a wide screen offers. The side-by-side stays for the same reason it was chosen: the tree is the page's picture and a picture beside its caption is a composition, while a picture under its caption is a poster. What was wrong with it was that it was stretched, and that is now fixed rather than abandoned.

### Three from the third look, 2026-09-01

**The caption is two lines by design now, not by wrapping.** Constraining it to the tree's width fixed the sideways drag and created a smaller problem: the break landed wherever the text ran out, which for the longest note was between a number and its unit - _"... Optimum - 623 / words - sapling"_. A line that breaks somewhere different for every note reads as a mistake each time. `to_html` now emits the note's name and its facts as two elements and the caption stacks them, so the break is decided in the generator rather than by whatever room was left. Two lines is exactly the room the stylesheet already holds open, and the second line still lands level with the colophon opposite it.

**The name sat above the tree.** The words column spanned the tools' row as well as the tree's, so the name began a tool's height above the top of the tree - a stagger nothing else on the page explains. The tools have a row of their own across the top of the grid now, and the name and the tree begin on the same line as well as finishing on the same line.

**The empty half of the left column got the site's links.** `footerLinks` - one map in the host's configuration, already rendered in the footer - now also renders under the site's sentence at the top of the home page, which is the one page that is about the site rather than about a note. One list rather than two, so "where else to find me" cannot say different things at the top and the bottom of the same page; adding a resume or a projects page is a line in `hosts/homelab01/default.nix` and nothing here. The one thing the option cannot express is order: it is an attribute set, so the links come out alphabetically, and a fixed order would mean making it a list of pairs.

**What the gate holds and what it cannot.** Two subtests: that the colophon's four numbers equal the stages in the staging tree — the failure that matters is the page set, since `site.RegularPages` silently including the landing page or the generated `/notes/` index would put the site's own furniture in its census — and that the composition is the home page's alone, with the small masthead title present on an essay and absent here. Whether a name at four times the size of anything else on the site reads as a mark or as a shout is still the screenshots' question, and the answer came from six widths in both themes, plus the three-note fixture for the small-garden case.

### A bar and a picture, 2026-09-02

Reported as four faults in one composition: the header hangs off the right of the page, the two columns are given the same room and only the tree fills its share, the phone layout centres the words for no reason the rest of the design gives, and the walk down from wide to narrow goes anchored, then centred, then stacked. Five alternatives were rendered and all five were declined, which was the useful result: they were five answers to _how should the header arrange itself_, and the answer is that it should not be arranging itself at all.

**The overhang was the anchor of 2026-09-01, half right.** That pass moved the header's left end onto the prose and its right end onto the wide grid's right edge, reasoning that it should end "where the page's widest element ends". The home page carries no sidenotes — checked, zero — so nothing on it ever reaches that edge. The rule under the header closed 288px past the last character of every paragraph below it, on a line the page never draws. Anchored to an empty column it reads as hanging off the page, which is a different fault at the same address, and it is why the previous entry's own measurement did not catch it: both ends _were_ on lines the layout has, and one of those lines is only ever drawn on an essay.

**The header is now exactly the article column, at every width.** That is the span every interior page's masthead rule already has, so the home page stops being the one page whose header is a different width from its body, and nothing can hang off it because there is nothing to its right. The probe in the VM test asserts both ends against the article's now, plus that the page is genuinely in its wide layout, so the assertion is about the anchoring and not about a narrow page where everything is one column anyway.

**The composition is a bar and a picture under it.** The name, the links and the tools share one line — the row the tools already had — and the tree has the whole measure beneath. The two parts stop being peers: the tree is the page's subject and the name is its label, and giving them equal columns is exactly what left one full and the other half empty. It costs the name, which goes from `4.25rem` to `2rem`; the interior masthead is `1.1rem`, so the landing page is still plainly the landing page. Three text blocks became one — the count moved under the tree and the sentence went — and the header is 640px wide and 531px tall at every width from 768px up, against 928×500 that reflowed twice on the way down.

**The transition is deleted rather than fixed.** There is no header breakpoint left. The header changes shape at 80rem because the _page_ does, at the moment the rail moves and the sidenotes appear, and below 34rem the bar breaks into name-and-tools over links, which is the same reading order at the same left edge. Nothing centres at any width except the caption and the count, which centre on the picture they describe. The 60rem stack and the 72rem centred band are both gone.

**The tree is sized from its column instead of from the window, and that is the whole fix for "only the tree fills its space".** The band was sized by the layout and the picture by the viewport (`clamp(0.5rem, 3.2vw, 1.25rem)`), and nothing made the two agree: at 1440px the tree stopped 74px short of its own rule. `.bonsai-plate` is now an inline-size container and the type is `100cqi / (--cols × 0.602)`, where `--cols` is written onto the `<pre>` by `bonsai.py` because only the generator can count it — asking the box how wide the type is would be the circle the sizing exists to break. Every vault fills the measure. `0.602` is DejaVu Sans Mono's advance and near enough Menlo's and SF Mono's; a narrower monospace (Consolas, 0.55) draws a tree a few percent short of the column rather than a broken one, and a wider one gets the `overflow-x` that was always there. The alternative was measuring the advance in script and correcting after paint, which buys those few percent for a layout shift on every load.

**The crown got wider because the picture was square.** At `CANOPY_ASPECT = 1.6` the whole tree — crown, bare trunk and pot — came out 35×18 cells, which on screen is 1.11:1. A square is the one shape that cannot be a masthead, and it is what forced the name beside the tree rather than above it in the first place. At 2.4 today's vault draws 43×15: wider, and three rows shorter, so the picture fills the measure without pushing the prose off the screen. That is 397px of tree height against 583px at the old proportion — 186px of page, for a shape the file's own comment already asked for ("a broad, shallow dome; a tall narrow one reads as a conifer"). It improves as the garden grows, too: the crown widens faster than it rises, so two hundred notes draw 78×27 rather than a taller and taller mass.

**The sentence went and nothing was lost.** `description.html` still resolves it for `<meta name="description">`, the social card and the feed. What it was not is prose written for this composition — the comment that used to sit beside it said as much — and the body's first paragraph says the same thing properly, in the site's own voice. A bar cannot hold a sentence.

**The count stayed, against the suggestion that it go too.** It is the one line of text on the page that describes the _picture_: eighteen notes is exactly what the eighteen clumps of foliage are, and `colophon.html` renders nowhere else, so cutting it would take the count off the site. Under the tree, on the caption's line, it reads as a plate's label rather than as a fourth block of masthead copy.
