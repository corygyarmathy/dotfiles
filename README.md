...
- **Nothing protects against a change that cuts a host off from outside.** Most verification runs on the host, so a machine that has lost its network still believes it is fine. Cross-host probes close most of the gap: each server probes the other's dedicated host-alive beacon (a dependency-free 200-responder, decoupled from every service), so a host that stops answering triggers the critical `HostUnreachableFromOutside` alert (see the [tunnel runbook](docs/runbooks/tunnel-and-probes.md)). The observer is a different machine, but the probe's path is not guaranteed: the fleet resolver normally answers `alive-*` with the LAN address, so the probe crosses the LAN rather than the public internet — a pure tunnel/egress fault is the one case it can miss, and that is deferred as a future item in the hardening plan. Recovery of the box itself still means a trip to the cupboard.
