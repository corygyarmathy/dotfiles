...
| `homelab01.yaml` | homelab01              | Everything only homelab01 declares — the tunnel, Grafana, miniflux, wallabag, the garden, and the AFK agent's GitHub App key and OpenCode credentials.                 |
...
- **A secret for a service that is switched off still lives in its host's file.** `vikunja/jwt-secret` and `digital-garden/deploy-key` are both parked in `homelab01.yaml` so that flipping the toggle works without an editing session, and so are the AFK agent's (`gh-ci/afk-agent-app-private-key`, `opencode/api-key`, `opencode/username`) — nothing declares them between the bash prototype's deletion and the Go agent's module (#281). `operator.yaml` is for what no host will ever declare, not for what no host declares today.
